...
def parse_args():
    #pars = argparse.ArgumentParser(description='get courses and clusters')
    pars = reqparse.RequestParser()
    pars.add_argument('-courses', nargs='+', type=str, default=False,
                        help='a course number')
    pars.add_argument('-cluster', type=str, nargs='+', action='append', default=[],
                        help='for each - group a group containing the courses will be added')
    pars.add_argument('-specific_windows', nargs='+', type=str, default=False,
                        help='for each specific window : add -spedific_window (day,period) like so (0,0) means:'
                             ' (yum aleph, 8:30-9:30)')
    pars.add_argument('-specific_days_off', nargs='+', type=str, default=False,
                        help='for each specific day off add: -specific_days_off day1 day2... like so -specific_days_off 0 4')
    pars.add_argument('-lecturer', nargs='+', type=str, default=False,
