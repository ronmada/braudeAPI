...
class Objective():
    ...
    def evaluation(self):
        ...
        :return:
        """
        return (self.max_objective - (self.Penalty()+self.fitness()))
    ...
    def spesific_window(self):
        ...
        if self.specific_windows:
            ...
            for item in Objective.specific_windows:
                if type(item)==str:
                    ...
                    for lect in kita.lectures:
                        if lect.day_in_week == day:
                            if lect.start_time <= window_start and lect.end_time >= window_end:
                                desecrated = True
                                print('specific window : ' + str(day) + ' ' + str(window_start) + ' desecrated')
                if desecrated:
                    spesific_window_desecrated+=1

        print ('specific windows desecrated ' + str(spesific_window_desecrated))
    ...
    def overlaps(self):
        ...
        print('overlaps are ' + str(overlaps))
        return overlaps
    ...
    def school_days(self):
        ...
        print ('amount of school days: ' + str(school_days))
        return school_days
    ...
    def specific_days_off(self):
        ...
        """
        days_violated = 0
        for day in Objective.specific_free_days:
            day = int(day)
            violated = False
            for kita in self.solution.lectures:
                for lect in kita.lectures:
                    if lect.day_in_week == day:
                        violated = True
            if violated:
                print ('fee day requested ' + str(day) + 'was violated')
                days_violated+=1
        print ('amount of free days violated : ' + str(days_violated))
    ...
    def classroom_proximity(self):
                        ...
                        if 'p' in last_room.lower():
                            if 'p' not in lect.location.lower():
                                if last_end_time.hour == lect.start_time.hour:
                                    far_classes+=1
                                    print ('far classes : '+str(lect.day_in_week) + ' ' + str(lect.start_time))
                        else:
        ...
        print('far classes: ' + str(far_classes))
        return far_classes
